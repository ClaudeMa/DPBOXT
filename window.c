...
extern int conv_umlaut_to_tx(char code,char *newcode);
...
void term_exit()
{
  if (termcap) {
    ...
    free(cbuf);
     ...
     free(WR); free(CL);
     free(IS); free(BCC); free(ND); free(CE);
...
int extcols;
{
 ...
 if (termcap) {
  ...
  /* Terminal Capabilities */
  if ((AL = tgetstr("al", &_tptr)) == NULL) {
    ins_line = 0;
  }
  else {
    ins_line = 1;
  }
  if ((DL = tgetstr("dl", &_tptr)) == NULL) {
    del_line = 0;
  }
  else {
    del_line = 1;
  }
  if ((IC = tgetstr("ic", &_tptr)) == NULL) {
    ins_char = 0;
  }
  else {
    ins_char = 1;
  }
  if ((DC = tgetstr("dc", &_tptr)) == NULL) {
    del_char = 0;
  }
  else {
    del_char = 1;
  }
  if ((CL = tgetstr("cl", &_tptr)) == NULL) {
  	printf("No clear screen capability (cl)\n");
  	return(1);
  }
  if ((IS = tgetstr("is", &_tptr)) == NULL)
    IS = "\033[r";
  RS = tgetstr("rs", &_tptr);
  if ((ND = tgetstr("nd", &_tptr)) == NULL) {
  	printf("No cursor right capability (nd)\n");
  	return(1);
  }
  if ((CE = tgetstr("ce", &_tptr)) == NULL) {
  	printf("No clear to end of line capability (ce)\n");
  	return(1);
  }
  if ((WR = tgetstr("cr", &_tptr)) == NULL)
    WR = "\015";
  if ((NL = tgetstr("nl", &_tptr)) == NULL)
    NL = "\012";
  if (tgetflag("bs") || ((BCC = tgetstr("bc", &_tptr)) == NULL))
    BCC = "\010";
  if ((CS = tgetstr("cs", &_tptr)) == NULL)
    scroll_reg = 0;
  else
    scroll_reg = 1;
...
struct window *win;
{
  ...
  if (column < COLS-1) column++; /* position at space after char */
  win->column = column;
  /* output on real screen ? */
  if (win->real != -1) {
    if ((win->line >= real_info[win->real].first_log_line) &&
...
struct window *win;
{
  ...
  for (i = 0; i < win->column; i++) {
    ...
    if (str[i] >= 0x80) {
      if (conv_umlaut_to_tx(str[i],&newchr))
        str[i] = newchr;
    }
  }
  str[i++] = CR;
